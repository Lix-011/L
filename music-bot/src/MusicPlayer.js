...
class MusicPlayer {
  ...
  async play(query, requester, voiceChannel) {
    ...
    if (validated === 'yt_video') {
      ...
      if (res.length > 0) {
        tracks.push({ title: `${sp.name} — ${sp.artists.map(a => a.name).join(', ')}`, url: res[0].url, duration: Math.floor(sp.durationInMs / 1000), thumbnail: sp.thumbnail?.url || res[0].thumbnails[0]?.url || null, requestedBy: requester });
      }
      ...
      for (const t of sp.tracks.slice(0, 30)) {
        ...
        if (res.length > 0) {
          tracks.push({ title: `${t.name} — ${t.artists.map(a => a.name).join(', ')}`, url: res[0].url, duration: Math.floor(t.durationInMs / 1000), thumbnail: t.thumbnail?.url || res[0].thumbnails[0]?.url || null, requestedBy: requester });
        }
      }
    } else {
  ...
  async seekForward(seconds) {
    ...
    this._startedAt  = Date.now();
    try {
      const stream = await playdl.stream(this.currentTrack.url, {
        quality: 2, discordPlayerCompatibility: true, seek: newOffset,
      });
